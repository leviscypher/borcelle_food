...
const login = async (e: any) => {
  ...
  if (dataAuth.username == '' && dataAuth.password == '') {
    ...
    try {
      const data = await axios.post('http://127.0.0.1:8000/api/login', dataAuth)
...
<template>
  <form id="form-login" class="flex justify-center flex-col w-3/4 mx-auto" @submit.prevent>
    <div class="form-group">
      <input type="text" placeholder="tên tài khoản" v-model="dataAuth.username"
    ...
    <div class="form-group relative mt-6">
      <input v-bind:type="showPassword ? 'text' : 'password'" placeholder="mật khẩu" v-model="dataAuth.password"
