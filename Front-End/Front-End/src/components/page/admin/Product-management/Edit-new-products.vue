...
onMounted(async () => {
  ...
  productId.value = id
  if (id) {
    await Promise.all([product.fetchEdit(id), category.fetchCategory(), productStatus.fetchProductStatus()])
    productEditData.value = product.getEditproducts
    products.name = productEditData.value.name
    products.price = productEditData.value.price
    products.quantity = productEditData.value.quantity
    products.description = productEditData.value.description
    products.categories_id = productEditData.value.categories_id
    products.product_status_id = productEditData.value.status_id
    selectedImages.value = productEditData.value.image_path
  }
})
...
const updateProduct = async () => {
  isLoader.value = true
  if (validate()) {
    await product.fetchUpdate(productId.value, products)
